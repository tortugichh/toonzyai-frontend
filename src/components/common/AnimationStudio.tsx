...
const AnimationStudio: React.FC<AnimationStudioProps> = ({ projectId }) => {
  ...
  // In API v2, generation requires mandatory segment_prompt,
  // so automatic launch for pending segments is disabled.
  // User must specify prompt manually in the SegmentEditor interface.
  ...
  const handleUpdatePrompt = (segmentNumber: number, prompt: string) => {
    // In API v2 there's no separate endpoint for updating prompts;
    // save locally to pass during generation.
  ...
  const handleGenerateAll = async () => {
    try {
      if (!currentProject) return; // safety check, but in UI it's always there
      ...
      if (invalid.length > 0) {
        toastError(`Specify prompts (≥10 characters) for segments: ${invalid.map(i => i.segment_number).join(', ')}`);
        return;
      }

      // 2) Update prompts in bulk on backend
  ...
  // Sum up progress of all segments for smoother progress bar
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600">
                Ready segments: {completedSegments.length} out of {currentProject.total_segments}
              </p>
